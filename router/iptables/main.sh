...
addRule "-t nat -A POSTROUTING -o $ext_iface -s $local_subnets" "FWD nat outbound" "nat all outbound packets" MASQUERADE
...
addRule "-A INPUT -i $ext_iface -d $local_subnets -m state --state RELATED,ESTABLISHED" "GENERAL inp rel" "GENERAL allow all input related"
...
addRule "-A FORWARD -o $ext_iface -s $local_subnets" "FWD outbound" "forward all outbound packets"

# allow inbound packets with context
addRule "-A FORWARD -i $ext_iface -d $local_subnets -m state --state RELATED,ESTABLISHED" "FWD inbound rel" "forward inbound packets with context"

############ DNS ###############

# allow requests coming from local subnets
addRule "-A INPUT -p udp --dport 53 -s $local_subnets" "DNS req from lan" "allow DNS requests from lan"
addRule "-A INPUT -p tcp --dport 53 -s $local_subnets" "DNS req from lan" "allow DNS requests from lan"
...
addRule "-A OUTPUT -p udp -d $local_subnets -m state --state RELATED,ESTABLISHED" "DNS repl to lan" "allow DNS replies to lan"
addRule "-A OUTPUT -p tcp -d $local_subnets -m state --state RELATED,ESTABLISHED" "DNS repl to lan" "allow DNS replies to lan"

############ CHRONY ###############

# allow requests from lan
addRule "-A INPUT -p udp --dport 123 -s $local_subnets -m state --state NEW" "NTP req from lan" "allow NTP requests from lan"

# allow replies to lan
addRule "-A OUTPUT -p udp --dport 123 -d $local_subnets -m state --state ESTABLISHED,RELATED" "NTP repl to lan" "allow NTP replies to lan"
